...
function toggleRegion(regionIndex) {
  const selectedRegion = payload["data"][0]["regions"][regionIndex]["shortname"];
  document.getElementById("region-label").innerText = selectedRegion;


  const selectedRegionData = payload["data"][0]["regions"][regionIndex]["generationmix"];
  
  const fuelTypes = selectedRegionData.map((mix) => mix["fuel"]);
  const fuelValues = selectedRegionData.map((mix) => parseFloat(mix["perc"]).toFixed(1));

  // Should update `current_region_by_energy` for region selected in dropdown
  current_region_by_energy = {};
  selectedRegionData.forEach((mix) => {
    current_region_by_energy[mix["fuel"]] = parseFloat(mix["perc"]).toFixed(1);
  });

  const filterEnergyChart = Chart.getChart("filter-energy-source");

  if (filterEnergyChart) {
    // Update the chart with the new fetched data
    filterEnergyChart.data.labels = fuelTypes;
    filterEnergyChart.data.datasets[0].data = fuelValues;
    filterEnergyChart.update();
  } else {
    console.error("Pie chart instance not found!");
  }

  console.log("Updated pie chart for:", selectedRegion, current_region_by_energy);
}
...
function populateCharts() {
  ...
  // Initialize `current_region_by_energy` for the first region
  const defaultRegionData = payload["data"][0]["regions"][0]["generationmix"];
  defaultRegionData.forEach((mix) => {
    current_region_by_energy[mix["fuel"]] = parseFloat(mix["perc"]).toFixed(1);
  });
}
...
function renderCharts() {
  ...
  // Initialize pie chart with the default region's data
  const fuelTypes = Object.keys(current_region_by_energy);
  const fuelValues = Object.values(current_region_by_energy);
  ...
  new Chart("filter-energy-source", {
    ...
    options: {
      ...
      title: {
        display: true,
        text: "Fuel Mix Distribution for Selected Region",
      },
    },
  });
